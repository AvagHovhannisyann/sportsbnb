/**
 * authKit — the shared vocabulary for the auth / onboarding / account family.
 * Not a composition: brand tokens, loop math and motion primitives that the 25
 * pieces in this folder import so they speak with one voice on /login, /signup,
 * /onboarding/* and the session-state surfaces.
 */
 ...
 * which is the theme the app actually ships. Written in comma form because
...
/**
 * The five rungs of the signup password-strength ladder, in the exact order
 * `getStrengthLabel` in `src/pages/SignupPage.tsx` returns them. The two middle
 * colours are Tailwind's `orange-500` / `yellow-500`, which that function names
 * directly; the top two are `--primary` at 70% and at full.
 */
export const STRENGTH_RUNGS = [
  { label: "Very Weak", color: "hsl(358, 72%, 68%)", upTo: 20 },
  { label: "Weak", color: "hsl(25, 95%, 53%)", upTo: 40 },
  { label: "Fair", color: "hsl(48, 96%, 53%)", upTo: 60 },
  { label: "Good", color: "hsla(151, 90%, 47%, 0.7)", upTo: 80 },
  { label: "Strong", color: "hsl(151, 90%, 47%)", upTo: 100 },
] as const;

/** Mirrors `getStrengthLabel`: score 0 has no verdict at all. */
export const strengthRung = (
  score: number,
): { label: string; color: string } => {
  if (score <= 0) return { label: "", color: BRAND.mutedForeground };
  for (let i = 0; i < STRENGTH_RUNGS.length; i += 1) {
    if (score <= STRENGTH_RUNGS[i].upTo) {
      return { label: STRENGTH_RUNGS[i].label, color: STRENGTH_RUNGS[i].color };
    }
  }
  return { label: "Strong", color: BRAND.primary };
};
 ...
 * headless Remotion render cannot reach the network.
 */
...
/**
 * Where stagger stops lengthening. Past the sixth sibling the extra delay stops
 * describing sequence and just makes the tail arrive late — the same cap
 * `LoginPage` and `SignupPage` apply.
 */
 ...
 * its range — the usual cause of an opacity of 1.3 or a negative radius when a
 * driver is a wrapped cycle rather than a raw frame. Clamping is what every
 * call in this folder wants, so it is the default here rather than four extra
 * lines at each site.
 */
 ...
 * embedded in the app gets a still frame instead. Auth is exactly where that
 * matters most — someone is trying to sign in, not watch a show.
 */
 ...
 * ("code accepted", "3 of 5 done"); freezing those at 0 would show an empty
 * component.
 */
...
export const eyebrowStyle = (unit: number): CSSProperties => ({
